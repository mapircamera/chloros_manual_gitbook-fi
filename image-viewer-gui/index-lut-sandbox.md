...
* **Interaktiivisen LUT-säätämisen** – hienosäädä väriasteikot ja -alueet
...
{% hint style=&quot;success&quot; %}
**Paras työnkulku**: Käytä Sandboxia kokeilemiseen ja optimaalisten indeksi- ja LUT-asetusten löytämiseen, ja sovella sitten näitä asetuksia koko tietojoukkoon projektin käsittelyn aikana.
{% endhint %}
...
Kun kuva on koko näytön kokoinen ja **Kuvankatseluohjelman** <img src="../.gitbook/assets/icon_image-viewer.JPG" alt="" data-size="line"> -välilehden sivupalkki on auki:
...
* **Tarkoitus**: Helpottaa kuvioiden näkemistä ja tulkintaa**Harmaasävy vs. väri-LUT:**
...
**Luokkien säätö**määrittää, kuinka monta erillistä värivaihetta gradientissa näkyy:**Luokkien lukumäärän vaihtoehdot:*** **2–5 luokkaa**: Erittäin laajat kategoriat, erilliset vyöhykkeet
* **6–10 luokkaa**: Tasapainoinen, sopii luokitteluun
* **11–20 luokkaa**: Tasaiset gradientit, jatkuva ulkonäkö
* **20+ luokkaa**: Lähes jatkuva, maksimaalinen tasaisuus**Säätöohjeet:**
...
4. Gradientti päivittyy **reaaliajassa** kuvassa**Vaikutus visualisointiin:*** **Vähemmän luokkia** (3–5): Luo erilliset alueet, yksinkertaistettu luokittelu, helpompi erottaa kategoriat
* **Keskikokoiset luokat** (6–10): Tasapainoinen lähestymistapa, sopii useimpiin sovelluksiin
* **Enemmän luokkia** (15–20): Pehmeät siirtymät, yksityiskohtaiset vaihtelut, valokuvamainen ulkonäkö**Käyttötarkoitukset:*** **Vähän luokkia (3–5)**: Esitysdia, luokittelukartat, yksinkertaiset raportit
* **Keskikokoiset luokat (6–10)**: Yleinen analyysi, tasapainoinen yksityiskohtaisuus, vakiomuotoiset raportit
* **Monet luokat (15–20)**: Tieteellinen analyysi, yksityiskohtainen tarkastelu, julkaisukelpoiset tulokset

### Arvoalueiden hienosäätö

**Arvoalueiden säätimet**määrittävät, mitkä indeksiarvot vastaavat mitäkin värejä gradientissasi:**Alueiden säätimet LUT-paneelissa:*** **Minimiarvo**: Väriskaalan alaraja
...
1. Etsi LUT-paneelista**Min Value**- ja**Max Value**-syöttökentät
...
6. Visualisointi **päivittyy välittömästi**{% vihje style=&quot;info&quot; %}**Automaattinen skaalaus**: Kun käytät LUT:ia ensimmäisen kerran, Chloros asettaa automaattisesti minimi-/maksimiarvon kuvan todelliseen data-alueeseen. Voit sitten kaventaa tätä aluetta keskittyäksesi tiettyihin kiinnostaviin arvoalueisiin.
{% endhint %}

**Esimerkki NDVI-alueen säätämisestä:*** **Koko alue**: `-1.0` – `1.0` (näytä kaikki mahdolliset arvot)
* **Kasvillisuuteen keskittynyt**: `0.2` – `0.9` (sulje pois paljas maaperä ja vesi)
* **Vain terve kasvillisuus**: `0.5` – `0.9` (korosta vain voimakkaat kasvit)
* **Stressin havaitseminen**: `0.2` – `0.5` (korosta ongelma-alueet)
* **Mukautettu alue**: Säädä havaitsemiesi pikseliarvojen perusteella**Miksi alueita säädetään?*** **Kontrastin lisääminen** kiinnostavalla alueella
...
Kun pikseliarvot ovat määritetyn minimi-/maksimialueen ulkopuolella, voit hallita niiden näyttötapaa **leikkaustiloilla**.
...
* **Alueella**olevat pikselit näkyvät**värigradienttina*** **Käyttötapaus**: Hienovarainen korostus, kontekstin säilyttäminen ja kiinnostavien alueiden korostaminen
* **Esimerkki**: Korosta stressaantunut kasvillisuus värillä (NDVI 0,3–0,5) ja näytä terveet alueet harmaana

#### 4. Alkuperäinen tausta

* **Alueen ulkopuolella**olevat pikselit näkyvät**alkuperäisenä monispektrisenä kuvana*** **Alueella**olevat pikselit näyttävät**värigradientin*** **Käyttötapaus**: Intuitiivisin – yhdistää luonnollisen kuvakontekstin analyyttiseen värikerrokseen
* **Esimerkki**: Katso todellinen pellon/sadon ulkonäkö värikoodatuilla stressialueilla

### Oikean leikkausmoodin valinta

| Leikkausmoodi              | Paras käyttötarkoitus                                   | Visualisointityyli          |
| -------------------------- | ------------------------------------------ | ---------------------------- |
| **Minimi ja maksimi**    | Täydellinen tietojen näyttö, tieteellinen analyysi     | Kaikki pikselit värillisiä           |
| **Läpinäkyvä tausta** | GIS-peittokuvat, tiettyjen alueiden erottaminen    | Väri alueella, tyhjä sen ulkopuolella |
...
7. Gradientti**päivittyy välittömästi** kuvaan**Väripysähdysten lisääminen tai poistaminen:*** **Lisää pysähdys**: Napsauta +-kuvaketta lisätäksesi uuden värinäytteen loppuun
* **Poista pysähdys**: Kaksoisnapsauta väriruutua poistaaksesi värinäytteen**Mukautusstrategiat:*** **Käännä gradientti**: Käännä värien järjestys päinvastaiseksi (esim. vihreä = matala, punainen = korkea)
...
* **Muokkaa värejä** → Mukautettu gradientti otetaan käyttöön välittömästi**Ei tarvita &quot;Käytä&quot;-painiketta** – kaikki muutokset ovat reaaliaikaisia ja interaktiivisia!
...
4. **Tarkista pikseliarvot** – Siirrä kursoria ympäriinsä, merkitse muistiin arvoalueet
...
7. **Hio värejä** - Mukauta gradienttia tarvittaessa tiettyjen korostusten vuoksi
8. **Viimeistele asetukset**- Dokumentoi asetukset ja kopioi ne projektin asetuksiin vientiä varten

### Pikseliarvojen tarkastus

Todellisten pikseliarvojen ymmärtäminen on ratkaisevan tärkeää tehokkaiden LUT-alueiden asettamiseksi:**Arvojen tarkastaminen:**
...
5. **Tee muistiinpanoja** eri ominaisuuksien arvoalueista:
   ...
   * **Vesi** (jos läsnä): esim. NDVI -0,05–0,10**Pikseliarvojen käyttäminen LUT-alueiden asettamiseen:**Tarkista pikseliarvot ja säädä LUT:n minimi- ja maksimiarvot vastaavasti:**Esimerkki:*** **Havainto**: Maaperän arvot = 0,05–0,25, stressaantunut = 0,25–0,50, terve = 0,50–0,85
...
{% hint style=&quot;warning&quot; %}
**Kaavan vahvistus**: Varmista, että kaavassa käytetään kamerassa käytettävissä olevia kaistoja. Esimerkiksi RedEdge on käytettävissä vain kameroissa, joissa on RedEdge-suodatin.
{% endhint %}
...
* **Joukkojen käsittely**: Käytä optimoituja indeksejä koko tietojoukkoon
...
* [**Kuvien käsittely (GUI)**](../processing-images-gui/adding-files-to-a-project.md) - Koko käsittelyn työnkulku
